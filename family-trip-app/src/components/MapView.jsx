...
export default function MapView({ data, onUpdateMapsConfig }) {
  ...
  const [mapCityFilter, setMapCityFilter] = useState('Seoul')
  ...
  const tabs = [
    { id: 'lists', label: 'Saved Maps' },
    { id: 'pins', label: 'Locations' },
    { id: 'map', label: 'Map' },
  ]
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="flex gap-2 p-3 overflow-x-auto shrink-0 bg-white border-b border-gray-100">
            {cities.map(city => (
              <button
                key={city}
                onClick={() => setMapCityFilter(city)}
                className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all whitespace-nowrap ${
                  mapCityFilter === city ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'
                }`}
              >
                {city}
              </button>
          ...
          <Suspense fallback={<div className="flex-1 flex items-center justify-center"><p className="text-sm text-gray-400">Loading map...</p></div>}>
            <PinMap cityFilter={mapCityFilter} />
          </Suspense>
        </div>
      )}
    </div>
